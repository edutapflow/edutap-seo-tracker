...
def run_automation():
    ...
    # 2. Build prev_map BEFORE the run using safe timestamp-filtered fetch
    print("   ... Building previous rank snapshot")
    prev_map = build_prev_map_safe()
    print(f"   ... Got previous ranks for {len(prev_map)} keywords")

    # 3. Run the Update — pass run_type="scheduled" so logs tab shows it correctly
    keywords_list = master_df.to_dict('records')
    date_str, cost, results_data = perform_update(
        keywords_list,
        progress_bar=None,
        status_text=None,
        run_type="scheduled"   # ← Shows as "Scheduled" in Run Logs tab
    )
    ...
    # Fetch DataForSEO balance right before sending email
