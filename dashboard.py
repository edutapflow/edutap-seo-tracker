# FORCE UPDATE V32 - FIXED PREV/CURR SWAP + BULK ACCURACY
...
def get_dashboard_view(master_df, history_df):
    ...
    if not history_df.empty:
        ...
        for kw, grp in history_df.groupby('keyword'):
            ...
            # ✅ FIX: prev_rank_map is built from the SECOND-MOST-RECENT unique day,
            # not second-to-last row. Using rows caused issues when multiple rows
            # existed per day (re-runs), making iloc[-2] grab same-day data as iloc[-1].
            unique_days_for_kw = grp['day'].unique()
            if len(unique_days_for_kw) >= 2:
                # Get the second most recent day's last rank
                second_day = sorted(unique_days_for_kw, reverse=True)[1]
                second_day_rows = grp[grp['day'] == second_day]
                prev_rank_map[kw] = int(second_day_rows.iloc[-1]['rank'])
            elif len(unique_days_for_kw) == 1:
                prev_rank_map[kw] = 101  # No previous day data yet
            else:
...
with tab1:
    ...
    if st.session_state['is_running']:
        if 'pending_update_list' in st.session_state:
            ...
            # ✅ FIX: Take prev_map snapshot RIGHT BEFORE running, using the safe
            # timestamp-filtered fetch. This prevents the new run's DB writes from
            # polluting the "previous rank" comparison, which was causing the
            # prev/curr swap seen in email alerts.
                ...
                with st.form("run_conf_form"):
                    ...
                    if submit:
                        if run_pass == st.secrets["RUN_UPDATE_PASSWORD"]:
                            ...
                            # ✅ FIX: Do NOT snapshot prev_map here — it's stale cached data.
                            # We now call build_prev_map_safe() at the moment of execution,
                            # not at the moment of button click. Removed the old line:
